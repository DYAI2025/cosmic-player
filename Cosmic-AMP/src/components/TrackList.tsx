...
export const TrackList: React.FC<TrackListProps> = ({
  ...
  tracks.forEach((track, index) => {
    ...
    const prevGroup = index > 0 ? tracks[index - 1].group : undefined;
    const showGroupHeader = Boolean(track.group && track.group !== prevGroup);

    if (showGroupHeader) {
      items.push(
        <p
          key={`group-${track.group}-${index}`}
          className="text-[9px] uppercase tracking-[0.3em] text-[var(--cp-text-muted)] px-1 pt-2 pb-0.5 border-t border-[var(--cp-border-subtle)]"
        >
          {track.group}
        </p>,
      );
          ...
          <div className="flex flex-col overflow-hidden">
            ...
            <span className="text-[10px] text-[var(--cp-text-muted)] font-mono">
              {track.duration
                ...
                : ''}
              {track.sizeBytes > 0
                ? `${Math.round((track.sizeBytes / 1024 / 1024) * 10) / 10} MB`
                : track.isPublic
                  ? 'Celestial Garden'
                  : ''}
            </span>
